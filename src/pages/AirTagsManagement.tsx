...
import { ArrowLeft, Plus, Edit2, Trash2, ChevronRight, Wind } from "lucide-react";
...
export default function AirTagsManagement() {
  ...
  const createTag = async () => {
    ...
    try {
      ...
      toast.success(`Air tag "${newTagName}" created`);
      setNewTagName("");
  ...
  const renderTag = (tag: Tag, level: number = 0) => (
    <div key={tag.id} className="space-y-2">
      <div
        className="flex items-center justify-between p-3 rounded-lg border border-air-secondary/20 bg-card hover:bg-air-primary/5 transition-colors"
        style={{ marginLeft: `${level * 24}px` }}
      >
        <div className="flex items-center gap-2">
          {level > 0 && <ChevronRight className="w-4 h-4 text-air-secondary" />}
          <Badge className="bg-air-light text-white border-air-secondary">{tag.name}</Badge>
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="ghost"
            size="sm"
            className="hover:bg-air-primary/10"
            onClick={() => {
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-8 md:px-12 lg:px-16 py-12">
        <div className="flex items-center justify-between mb-6">
          <Button variant="ghost" onClick={() => navigate("/?type=air")}>
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back
          </Button>
          <div className="flex items-center gap-2">
            <Wind className="w-6 h-6 text-air-primary" />
            <h1 className="text-2xl font-bold">Manage Air Tags</h1>
          </div>
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="p-6 rounded-lg border border-air-secondary/20 bg-card space-y-4">
            <h2 className="text-lg font-semibold">
              {parentTagForNew ? `Create Child Tag for "${parentTagForNew.name}"` : "Create New Air Tag"}
            </h2>
            {parentTagForNew && (
              <div className="flex items-center gap-2">
                <span className="text-sm text-muted-foreground">Parent:</span>
                <Badge className="bg-air-light text-white border-air-secondary">{parentTagForNew.name}</Badge>
                <Button
                  variant="ghost"
                  size="sm"
            ...
            <div className="flex gap-2">
              <Input
                ...
                className="border-air-secondary/20 focus:border-air-primary"
              />
              <Button 
                onClick={createTag} 
                disabled={!newTagName.trim()}
                className="bg-air-primary hover:bg-air-secondary text-white"
              >
          ...
          <div className="space-y-2">
            {isLoading ? (
              ...
              <div className="text-center py-12 text-muted-foreground">No air tags yet</div>
            ) : (
        ...
        <Dialog open={!!editingTag} onOpenChange={() => {
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Edit Air Tag</DialogTitle>
