...
import { ArrowLeft, Plus, Edit2, Trash2, ChevronRight, Circle } from "lucide-react";
...
export default function VoidTagsManagement() {
  ...
  const createTag = async () => {
    ...
    try {
      ...
      toast.success(`Void tag "${newTagName}" created`);
      setNewTagName("");
    ...
    <div key={tag.id} className="space-y-2">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="bg-muted">{tag.name}</Badge>
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-8 md:px-12 lg:px-16 py-12">
        <div className="flex items-center justify-between mb-6">
          <Button variant="ghost" onClick={() => navigate("/?type=void")}>
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back
          </Button>
          <div className="flex items-center gap-2">
            <Circle className="w-6 h-6 text-muted-foreground" />
            <h1 className="text-2xl font-bold">Manage Void Tags</h1>
          </div>
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="p-6 rounded-lg border bg-card space-y-4">
            <h2 className="text-lg font-semibold">
              {parentTagForNew ? `Create Child Tag for "${parentTagForNew.name}"` : "Create New Void Tag"}
            </h2>
            {parentTagForNew && (
              <div className="flex items-center gap-2">
                <span className="text-sm text-muted-foreground">Parent:</span>
                <Badge variant="outline" className="bg-muted">{parentTagForNew.name}</Badge>
                <Button
                  variant="ghost"
                  size="sm"
          ...
          <div className="space-y-2">
            {isLoading ? (
              ...
              <div className="text-center py-12 text-muted-foreground">No void tags yet</div>
            ) : (
        ...
        <Dialog open={!!editingTag} onOpenChange={() => {
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Edit Void Tag</DialogTitle>
